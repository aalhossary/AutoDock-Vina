// QuickVina's stores of already-visited points in conformation space.
//
...
// needs, and makes a stored point cheap. Both products share it verbatim.
//
// `visited` is QuickVina 2's store (QVina_2.1 `visited.h:75`): a fixed-capacity
// ring of 5n points, of which the 4n nearest are consulted.
//
// `circularvisited` is QuickVina-W's replacement for it (`visited.h:337`) --
// the same ring, but with the `excluded` argument and the integer return that
// let a consult be split across two stores. W declares it
// `class circularvisited /*: public visited*/`, the base class this port
// supplies as `search_database`.
//
// `Octree` is the store QuickVina-W adds on top: one per process, shared by
// every thread behind a readers-writer lock, holding the endpoints of the
// authentic-potential refinements. That is the "inter-process spatio-temporal
// integration" of the QuickVina-W paper.
//
// QuickVina 2 uses `visited` alone; QuickVina-W uses `Octree` first and
// `circularvisited` second. Stock Vina uses neither.
...
// QuickVina-W's shared database is read by every thread and written by any of
// them, so it carries a readers-writer lock.
...
// QuickVina-W's shared search database: an octree of endpoints, indexed by the
// ligand's position, readable and writable by every thread.
//
// This is what "inter-process spatio-temporal integration" means concretely
// (Hassan et al., Sci Rep 2017). The per-task ring above lets a thread avoid
// re-exploring what IT has already seen; this lets it avoid re-exploring what
// ANY thread has seen. The paper's split: "having history from other threads
// allows us to make use of other threads experience and make decisions in
// already explored energy landscape areas, while having history from an
// individual same thread allows us to make decisions in virgin areas".
//
// It holds endpoints only -- "the output of last iteration of local
// optimization (also known as end points or stop points)" -- which is why
// bfgs() adds to it after the loop rather than inside it, and only for the
// authentic-potential refinement.
//
// R = 5 A proximity cutoff, WMIN = 0.1 A minimum cell, SMAX = 8 points per node.
// Being a process-wide singleton with shared mutable state, it is also why
// QuickVina-W is not reproducible from a seed above one thread.
struct Vec3 {
	union {
		struct {
			float x,y,z;
...
class Octree : public search_database {
